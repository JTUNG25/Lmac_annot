...
# Pipeline settings
RUN_INTERPROSCAN = False  # Set to True if you want domain analysis (adds 3+ hours per mutant)
MAX_EVALUE       = 1e-5
...
rule interproscan:
    input:
        proteins = "results/sequences/{mutant}/proteins.fasta",
    output:
        results = "results/interproscan/{mutant}/interproscan_results.tsv" if RUN_INTERPROSCAN else [],
    log:
    ...
    shell:
        ...
        interproscan.sh \
            ...
            --cpu {threads} \
            > {log} 2>&1
        """ if RUN_INTERPROSCAN else "echo 'InterProScan skipped' > {log}"
...
rule combine_annotations:
    input:
        ...
        interproscan = "results/interproscan/{mutant}/interproscan_results.tsv" if RUN_INTERPROSCAN else [],
        gene_list    = lambda wildcards: MUTANTS[wildcards.mutant],
    output:
...
rule summarize_all_mutants:
    input:
        annotations = expand("results/annotation/{mutant}/final_annotation.xlsx",
                             mutant=mutant_names)
    output:
    ...
    run:
        ...
        for i, mutant in enumerate(mutant_names):
            try:
                ...
                df["Mutant"]      = mutant
                df["Mutant_Type"] = "_".join(mutant.split("_")[:-1])  # ago1, dcl2, etc.
                df["Effect_Type"] = mutant.split("_")[-1]             # silencing / derepression
                ...
                summary_row = {
                    ...
                    "Total_Genes":     len(df),
                    "With_Function":   df["Predicted_Function"].notna().sum()
                                       if "Predicted_Function" in df.columns else 0,
                    "With_GO_Terms":   df["GO_Terms"].notna().sum()
                                       if "GO_Terms" in df.columns else 0,
        ...
        if all_data:
            ...
            with pd.ExcelWriter(output.summary, engine="openpyxl") as writer:
                combined_df.to_excel(writer, sheet_name="All_Annotations", index=False)
                summary_df.to_excel(writer,  sheet_name="Mutant_Summary",  index=False)

                if len(summary_df) > 0:
                    pivot_genes = summary_df.pivot(
                        index="Mutant_Type", columns="Effect_Type", values="Total_Genes"
                    ).fillna(0)
                    pivot_annotated = summary_df.pivot(
                        index="Mutant_Type", columns="Effect_Type", values="With_Function"
                    ).fillna(0)
