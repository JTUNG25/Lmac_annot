        ...
        for i, mutant in enumerate(mutant_names):
            try:
                ...
                summary_row = {
                    ...
                    "Total_Genes": len(df),
                    "With_Function": (
                        df["Functional_Description"].notna().sum()
                        if "Functional_Description" in df.columns
                        else 0
                    ),
                    "With_GO_Terms": (
                        df["GOs"].notna().sum() if "GOs" in df.columns else 0
                    ),
                    "With_KEGG": (
                    ...
                    "With_COG": (
                        df["COG_category"].notna().sum()
                        if "COG_category" in df.columns
                        else 0
                    ),
                    "With_DIAMOND_Hit": (
                        df["Subject_Description"].notna().sum()
                        if "Subject_Description" in df.columns
                        else 0
                    ),
                }
